...
const promovisi = async(id, onPromotionComplete) => {
  try {
    console.log(id);
    // Use template literals to append the query parameter to the URL
    const response = await axios.put(`http://localhost:8000/promoteUser?prom_id=${id}`, null, {
      headers: { 'Content-Type': 'application/json' }
    });
...
const Row = ({ row, onDeleteProject }) => {
  ...
  const handleEdit = async () => {
    await promovisi(row.id, onDeleteProject);
  };
  ...
  return (
    <div className='main-container'>
      <div className='listContainer'>
        <div className='listTitle'>Useri</div>
